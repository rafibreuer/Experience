...
//new user({ name: 'rafi', userName: { name: 'rybtester@gmail.com', password: '1234' }, perm: 'admin' }).save();
app.use(auth(app));
app.get('/', (req, res) => {
  const name = { "name": "Sample", "type": "Warehouse Manager" };
  bin.find({}, {
    number: 1
  }).
    then((result) => {
...
app.post('/deleteUser', (req, response) => {
  user.findOne({ "userName.name": req.body.number }).then(res => {
    if (res.perm === 'admin') {
      if (res.userName.name === process.env.EMAIL) { response.end(); return; }
