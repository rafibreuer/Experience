...
import validatePhone from '../../utils/checkPhone';
...
class Input extends React.Component<InputProps & InputReduxProps, InputState>{
    ...
    validateInput = () => {
        ...
        if (val && this.props.name === 'phone' && !validatePhone(val)) {
            state.hasError = true;
            state.errorMessage = 'Not valid phone Number';
        }
        if (val && this.props.name === 'url' && !(/^(ftp|http|https):\/\/[^ "]+$/.test(val))) {
            state.hasError = true;
            state.errorMessage = 'Not valid URL';
        }
        this.setState(state);
    }
    checkIfSubmitted = () => {
